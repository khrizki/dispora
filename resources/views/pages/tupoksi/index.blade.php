                            ...
                            <tbody class="text-gray-600 fw-semibold">
                                @foreach($tupoksi as $index => $item)
                                    <tr>
                                        ...
                                        <td>
                                            {{-- hitung poin fungsi berdasarkan baris --}}
                                            {{ count(explode("\n", trim($item->fungsi))) }} Poin
                                        </td>
                                        <td>{{ $item->urutan }}</td>
                                        <td class="text-end">
                                            <a href="{{ route('pages.tupoksi.edit', $item->id) }}" class="btn btn-light btn-active-light-primary btn-sm">Edit</a>

                                            <form action="{{ route('pages.tupoksi.destroy', $item->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
